...
from landmarks import SMPL_INDEX_LANDMARKS, SMPLX_INDEX_LANDMARKS
...
class SMPLXBodyModel():
    ...
    @property
    def verts(self):
        # For experimental support, use basic parameters
        # TODO: Add proper hand and face parameter support
        return self.body_model(body_pose=self.current_pose, 
                               betas=self.current_shape, 
                               global_orient=self.current_global_orient).vertices[0]
    ...
    def __call__(self, pose, betas, **kwargs):
        ...
        body_pose = pose[:,3:]
        global_orient = pose[:,:3]
        # For experimental support, use basic SMPL-X parameters
        return self.body_model(body_pose=body_pose, 
                               betas=betas, 
                               global_orient=global_orient)
